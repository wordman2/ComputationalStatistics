...
```
Nach den t-Tests gibt es einen signifikanten Unterschied zwischen den Mittelwerten der Simulationen und der Mittelwerte des Bootstrapping. Die Bootstrap-Methode basiert auf der Annahme, dass die Stichprobe (der Datensatz vom Umfang 500) eine gute Schätzung für die zugrundeliegende Gleichverteilung ist. Wie an den bereits generierten Plots zu sehen ist, ist die Stichprobe größtenteils über dem erwarteten Wert 0.5 und daher gleichen die Mittelwerte der Bootstrap-Methode nicht den simulierten Mittelwerten.
...
col_names <- c("n", "LL","UL", "Error_LL", "Error_UL")
...
for(i in seq_along(coef_names))
  {
    ...
    for(ni in n_bootstrap){
      ...
      df <- rbind(df,c(ni,ci, ci-ci_lm[i,]))
    }
...
Je höher die Anzahl der Replikationen ist, desto stabiler ist in der Regel die Schätzung für das Konfidenzintervall. 

```{r}

n <- 1000
options <- c("basic", "bca", "perc")
col_names <- c("n", "LL","UL", "Error_LL", "Error_UL", "options")
...
for(i in seq_along(coef_names))
  {
    ...
    for(x in options){
      ...
      df <- rbind(df,c(ni,ci, ci-ci_lm[i,], x))
    }
    colnames(df) <- col_names
    print(df)
  }
