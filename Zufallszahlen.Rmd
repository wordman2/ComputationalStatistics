...
poisson_random_r <- rpois(n, lambda = 3) # random generated poisson-distributed numbers in R
...
ggplot() +
  geom_qq(aes(sample = poisson_random_inversion, color="inverse Poisson-Funktion"), alpha=0.5) +
  geom_qq(aes(sample = poisson_random_r, color = "R generierte Poisson-Verteilung"), ,alpha=0.5) +
...
exponential_random_inversion <- (-1/lambda) * log(1 - uniform_random)
exponential_random_r <- rexp(n, rate = lambda)

ggplot()+
  geom_histogram(aes(exponential_random_inversion, fill="blue"), alpha=0.5) +
  geom_histogram(aes(exponential_random_r, fill="red"), alpha=0.5) +
  labs(title = "Exponential Distribution") +
  theme_minimal()


ggplot() +
  geom_qq(aes(sample = exponential_random_inversion), color = "blue") +
  geom_qq(aes(sample = exponential_random_r), color = "red") +
  labs(title = "QQ-Plot - Exponential Distribution",
       x = "Theoretical Quantiles",
       y = "Sample Quantiles") +
  theme_minimal()
...
ggplot() +
  geom_qq(aes(sample = z), color = "blue") +
  geom_qq(aes(sample = normal_random_r), color = "red") +
...
hist(uniform_own, main="The Last Chaos Digits")
plot(embed(uniform_own, 2), main="The Last Chaos Digits")
