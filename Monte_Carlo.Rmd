...
for(i in k){
  ...
  Tscheb_value = (4*pi-pi^2)/(abs(z-pi)^2*alpha)

  tschebyscheff <- c(tschebyscheff, Tscheb_value)
}
result <- data.frame(n=n, approximation=approximationen, error=error, tschebyscheff=tschebyscheff)
print(result)

```

Nach der Tschebyscheffschen Ungleichung ist das n für eine 95%-Wahrscheinlichkeit den Fehler zu erreichen immer deutlich höher als das gewählte n.
